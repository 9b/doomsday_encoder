...
class kenai {
	...
	public function play_fetch() {
	    $rnd_nm_crypt = $this->get_random_string_array(rand(30, 100), 50);
	    $this->fetch_url = $rnd_nm_crypt[0] . ".php";
	    $fname = $rnd_nm_crypt[0] . ".php";
		$dmp = "";
		$dmp .= "<?php\n";
		$dmp .= "include_once 'database/database_connection.php';\n";
		$dmp .= "\$val = \$_POST['q'];\n";
		$dmp .= "\$key = addslashes(\$_POST['s']);\n";
		$dmp .= "\$query = \"SELECT alpha,count FROM seed_tokens WHERE seed='\$key';\";\n";
		$dmp .= "\$result= mysqli_query(\$link,\$query);\n";
		$dmp .= "\$row = mysqli_fetch_assoc(\$result);\n";
		$dmp .= "if(\$row['count'] >= 0) {\n";
		$dmp .= "\$data = strpos(\$row['alpha'],\$val);\n";
		$dmp .= "\$ncount = \$row['count'] - 1;\n";
		$dmp .= "\$query = \"UPDATE seed_tokens set count = \$ncount WHERE seed='\$key';\";\n";
		$dmp .= "\$result= mysqli_query(\$link,\$query);\n";
		$dmp .= "} else {\n";
		$dmp .= "\$data = rand(0,500);\n";
		$dmp .= "}\n";
		$dmp .= "header('Content-type: text/json');\n";
		$dmp .= "echo json_encode(\$data);\n";
		$dmp .= "?>";
	...
	public function over_and_out() {
		$fetch = $this->get_fetch_url();
		$fp = fsockopen("127.0.0.1", 80, $errno, $errstr, 30);
	    $out = "GET /doomsday_encoder/orangegift/register.php?unlink=$fetch HTTP/1.1\r\n";
	    $out .= "Host: www.9bplus.com\r\n";
	...
	public function gen_key() {
		$this->var_holder = $this->get_random_string_array(rand(30, 100), 50);
		return $this->var_holder[42];
	}
	
	public function gen_seed($key) {
		$seed = 0;
		$letter = $key[0];
		$seed += ord($letter) + strlen($key);
		return $seed;
	}

	public function calc_val($seed,$buf,$out) {
		if($buf != "") {
			$letter = $buf[0];
			$buf = substr($buf,1,strlen($buf));
			$remain = $seed % 94;
			$l_code = ord($letter);
			$o_code = $l_code - $remain;
			...
			$out[] = chr($o_code);
			$seed = $l_code + $remain;
			return $this->calc_val($seed,$buf,$out);
		} else {
			return $out;
		}
	}

	public function cleaner($buf) {
		$buf = str_replace("\\","\\\\",$buf);
		$buf = str_replace("'","\\'",$buf);
		$this->full_blob = $buf;
	}

	public function get_full_blob() {
		return $this->full_blob;
	}
	
	public function set_changers() {
		$valid_encodes = array(chr(32),"!","@","#","$","%","^","&","*","(",")","~","-","+","=",".",">","<","/","`","?",":",";"); //chr(9),chr(11),chr(12),
...
//		$payload .= "$rnd_nm_crypt[15]";
	...
	public function exta_encoder($code) {
		$key = $this->gen_key();
		...
		$payload .= "$rnd_nm_crypt[2] += $rnd_nm_crypt[1].length;";
		$payload .= "$rnd_nm_crypt[3] = '$clean_encoded';";
		$payload .= "var $rnd_nm_crypt[4] = '';";
		$payload .= "for($rnd_nm_crypt[5]=0;$rnd_nm_crypt[5]<$rnd_nm_crypt[3].length;$rnd_nm_crypt[5]++){";
		$payload .= "$rnd_nm_crypt[2] = $rnd_nm_crypt[2] % 0x5e;";
		$payload .= "$rnd_nm_crypt[6] = $rnd_nm_crypt[3].charCodeAt($rnd_nm_crypt[5]) + $rnd_nm_crypt[2];";
		$payload .= "if($rnd_nm_crypt[6] >= 0x7e) {";
		$payload .= "$rnd_nm_crypt[6] = $rnd_nm_crypt[6] - 0x5e;";
		$payload .= "}";
		$payload .= "$rnd_nm_crypt[4] += String.fromCharCode($rnd_nm_crypt[6]);";
		$payload .= "$rnd_nm_crypt[2] += $rnd_nm_crypt[6];";
		$payload .= "}";
		$payload .= "return $rnd_nm_crypt[4];";
		$payload .= "}";
		$payload .= "var $rnd_nm_crypt[7] = $rnd_nm_crypt[0]();";
		$payload .= "$rnd_nm_crypt[8]['\x77\x72\x69\x74\x65']('<scri'+'pt>');";
		$payload .= "$rnd_nm_crypt[8]['\x77\x72\x69\x74\x65']($rnd_nm_crypt[7]);";
		$payload .= "$rnd_nm_crypt[8]['\x77\x72\x69\x74\x65']('</scri'+'pt>');";
		return $payload;
	}
}
